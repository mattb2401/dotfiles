...
function handleItem($item) {
    if (strpos($item, "app".DIRECTORY_SEPARATOR."cache") !== FALSE)
        return;
    if (is_dir($item)) {            
...
class FileReader
{
    ...
    function getKeyword($type, $line) {
        $typepos = strpos(strtolower($line), $type);
        if ($typepos !== 0 && $line[$typepos-1] !== ' ' && ($line[$typepos-1] !== "\t"))
            return NULL;
